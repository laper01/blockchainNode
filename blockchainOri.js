...
class Transaction{
    ...
    signTransaction(signingKey){
        if(signingKey.getPublic('hex') !== this.fromAddress){
            throw new Error ('YOU cannot sign transation for other Wallet');
        }
...
class Blockchain{
    ...
    addTransaction(transaction){
        ...
        if(!transaction.fromAddress || !transaction.toAddress){
            throw new Error('Transaction does no have address')
        }

        if(!transaction.isValid()){
            throw new Error('trasaction invalid')
        }
        // just push to pendding transaction
